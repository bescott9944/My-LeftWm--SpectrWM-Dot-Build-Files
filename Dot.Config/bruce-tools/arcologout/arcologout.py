...
class TransparentWindow(Gtk.Window):
    ...
    wallpaper = "~/work/arcologout/wallpaper.jpg"
    d_buttons = ['cancel',
                 'shutdown',
                 'restart',
                 'suspend',
    ...
    def __init__(self):
        ...
        if not fn.os.path.isdir(fn.home + "/.config/bruce-tools/arcologout"):
            fn.os.mkdir(fn.home + "/.config/bruce-tools/arcologout")

        if not fn.os.path.isfile(fn.home + "/.config/bruce-tools/arcologout.conf"):
            shutil.copy(fn.root_config, fn.home + "/.config/bruce-tools/arcologout.conf")
    ...
    def on_save_clicked(self, widget):
        ...
        try:
            with open(fn.home + "/.config/bruce-tools/arcologout.conf", "r") as f:
                lines = f.readlines()
                f.close()
            ...
            with open(fn.home + "/.config/bruce-tools/arcologout.conf", "w") as f:
                f.writelines(lines)
                f.close()
            self.popover.popdown()
        except Exception as e:
            fn.os.unlink(fn.home + "/.config/bruce-tools/arcologout.conf")
            if not fn.os.path.isfile(fn.home + "/.config/bruce-tools/arcologout.conf"):
                shutil.copy(fn.root_config, fn.home + "/.config/bruce-tools/arcologout.conf")
        ...
        if (data == self.binds.get('logout')):
            ...
            if not fn.os.path.isdir(fn.home + "/.cache/i3lock"):
                if fn.os.path.isfile(self.wallpaper):
                    ...
                    self.lbl_stat.set_markup("<span size=\"x-large\"><b>Choose a wallpaper with Bruce''arcolinux-betterlockscreen</b></span>")  # noqa
