...
working_dir = ''.join([str(Path(__file__).parents[2]), "/bruce-tools/arcologout-themes/"])
# config = "/etc/arcologout.conf"
if os.path.isfile(home + "/.config/bruce-tools/arcologout.conf"):
    config = home + "/.config/bruce-tools/arcologout.conf"
#else:
...
def get_config(self, Gdk, Gtk, config):
    try:
        ...
        print(e)
        os.unlink(home + "/.config/bruce-tools/arcologout.conf")
        if not os.path.isfile(home + "/.config/bruce-tools/arcologout.conf"):
            shutil.copy(root_config, home + "/.config/bruce-tools/arcologout.conf")
