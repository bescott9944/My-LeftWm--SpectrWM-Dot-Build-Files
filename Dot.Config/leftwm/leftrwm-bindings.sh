...
printf ${RED}${bold}" ALT (${GREEN}MoD-Key${RED}${bold}) KEY BINDINGS FOR SPECTRWM\n"${NC}${normal} > $HOME/.config/leftwm/bindings.txt
grep "=" $HOME/.config/leftwm/config.toml | grep value| awk {'print $1,$3'} | sed 's/value//g' | sed 's/"//g' | sed 's/=//g'
grep 'modifier' $HOME/.config/leftwm/config.toml | awk {'print $3,$4'} #| column # | grep key | awk {'print'} # | column  >> $HOME/.config/leftwm/bindings.txt
grep 'key' $HOME/.config/leftwm/config.toml | awk {'print $3'}

# cat $HOME/.config/leftwm/bindings.txt
